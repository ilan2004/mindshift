...
(function () {
  const REQ = "nudge:focus";
  const RES = "nudge:focus:status";
  ...
  window.addEventListener("nudge:focus:start_template", (e) => {
  ...
  window.addEventListener("nudge:focus:microbreak", (e) => {
  ...
  window.addEventListener("nudge:focus:break_confirmed", () => {
    dispatchFocus("startBreak", { durationMinutes: 5 });
  }, false);

  // Temporary allow a domain for N minutes: detail { url, minutes }
  window.addEventListener("nudge:blocker:allow_temp", (e) => {
    const detail = (e && e.detail) || {};
